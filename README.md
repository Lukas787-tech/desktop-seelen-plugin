...
3. **Switches them on** in Seelen's settings. Seelen holds its settings in memory, so when something
   ...
   `settings.json.before-desktop-surface`.
4. **Optionally installs the assistant's local voice** (Kokoro, English and German, about 4 GB; see
   [Voice](#voice)), fetching `uv` with winget first if it is not there.

Everything is per user, so setup itself never asks for administrator rights; winget does, if it has
to install Seelen UI. It is listed in *Settings -> Apps -> Installed apps* and uninstalls from there,
which removes the widgets and the theme, offers to remove the voice and the surface's own data, and
leaves Seelen UI and its settings alone.

The installer is not code-signed, so SmartScreen may warn about it: *More info -> Run anyway*. Each
release lists the file's SHA-256 to check it against. For scripted installs:

```
SeelenDesktopSurface-Setup-<version>.exe /quiet [/voice] [/notheme] [/log setup.log]
SeelenDesktopSurface-Setup-<version>.exe /uninstall /quiet
```

`/quiet` answers every question yes and exits with 0 when done, 1 on failure, and 2 when Seelen UI
is missing or too old. The assistant itself still needs a model: its panel walks through Ollama or
an API key the first time it opens.
...
(or `assistant-local`, `-agent`, `-voice`, `-tools`, `-memory`) opens the settings on that tab, and
`window.__voice` plays recorded clips through the real recogniser, since a preview tab has no
microphone. The voice mode's pure parts - reading a reply aloud, cutting it while it streams, the
transcript cleanup, the resampler and WAV encoder, the end-of-speech detector and every engine's
request shape - are `scripts/test/voice.test.mjs`.

#### Voice

Three ways in, all from the panel: the microphone in the message box dictates into it; the waveform
button that stands in for *Send* while the box is empty starts a hands-free conversation; and
*Listen* on a reply reads it aloud. In a conversation the microphone stays open, a pause ends your
turn, the reply is spoken sentence by sentence while it is still being written, and talking over it
stops it and listens. An approval card is read out, and a spoken yes or no - *ja* and *nein* too -
answers it. Everything stays on this machine.

- **Listening is the chat model itself.** `gemma4:12b` lists `audio` in `/api/show`, and Ollama's
  native `/api/chat` takes a 16 kHz WAV in a message's `images`. Measured here: English and German
  clips came back word for word in 0.6-0.8 s warm for 4-5 s of speech, and so did Kokoro's own
  English and German output played back into it. Three things had to be learned first:
  - **The request has to carry the chat's own `num_ctx` and `keep_alive`**, or Ollama reloads the
    model to change them - 16 to 85 s instead of under one. The OpenAI shim's `input_audio` works
    too, but cannot carry `num_ctx`, so it reloaded every time.
  - **It repeats itself, and it refuses silence.** A German clip came back as the same sentence on
    two lines, and three seconds of faint noise as *"I'm sorry, but I cannot fulfill this request. I
    am a text-based AI and cannot hear or process audio files."* `cleanTranscript` in
    `src/lib/speech.ts` removes both, and the end-of-speech detector means silence is rarely sent.
  - **A clip is capped at 28 s**, under Gemma's 30 s audio window; longer dictation is cut there and
    transcribed piece by piece, in order.
- **Speaking is a local Kokoro server**, `scripts/voice/`, installed with `npm run voice:install`
  into `%USERPROFILE%\.ralfm-voice`: Kokoro-82M for English, and the `kikiri-german-martin`
  fine-tune of it for German, because Kokoro itself has no German voice. The fine-tune shares
  Kokoro's config and vocabulary exactly, so both load into one process behind OpenAI's
  `/v1/audio/voices` and `/v1/audio/speech`. Measured on the RTX 5060 Ti: ready 13 s after it is
  started, then about 110 ms to synthesise 3.3 s of speech, English or German. Every piece of a reply
  gets the voice for the language it is in, guessed from its commonest words, so a German answer is
  never read by an English voice.
  - It listens on `127.0.0.1` only, and answers CORS for `http://tauri.localhost` and localhost pages
    and nobody else - checked with a preflight from another origin, which gets no header back. Hugging
    Face is put in offline mode once the models are there.
  - **The assistant starts it** through `start.ps1` when voice is used and nothing answers, and it
    exits after an idle hour to give back its ~2.5 GB of memory and its VRAM. Nothing starts with
    Windows, and deleting the folder uninstalls it.
  - **The German weights did not load, and nothing said so.** The fine-tune was saved by a newer
    PyTorch, which names a weight-normalised layer's tensors `parametrizations.weight.original0/1`
    where Kokoro's layers take `weight_g`/`weight_v`; kokoro's loader then falls back to
    `strict=False` and skips all 178 of them. The voice still played - noise: gemma answered *"there is
    no speech in the provided file"*, and the clip crossed zero 0.43 times a sample against English
    speech's 0.12. The server maps the names and loads strictly, and gemma now transcribes the German
    test sentences back word for word, `13. September um 18:30 Uhr` included.
  - **Started without a window, it crashed on import.** `pythonw` gives a process no console, so
    `sys.stderr` is `None`, and kokoro's logger setup raises on a `None` sink. The server now hands
    headless output streams a file, `server.out.log`, before anything is imported.
  - **It is not in AppData on purpose.** Installed from a terminal inside a packaged app - the Claude
    desktop app, as it happened - every AppData write is silently redirected into that app's own
    container, where Seelen's PowerShell never looks. The first install landed there, 4.5 GB of it.
  - Any OpenAI-compatible speech server can stand in (Kokoro-FastAPI, Speaches, LocalAI), and a
    Whisper server can listen instead of the model. With *Local models only* off, Groq, OpenAI and
    Gemini can listen and OpenAI and Gemini can speak. Windows' own voices are the fallback that is
    always there, and sound like it.
- **A reply is cut while it streams** (`SpeechChunker`): the first sentence goes the moment it ends,
  later ones gather into pieces of about 90 characters, and up to two are synthesised ahead of the
  one playing. A full stop only ends a sentence once whitespace follows it, so `3.` is not spoken
  before it becomes `3.5`, and neither `13. September` nor `z.B.` ends one. Markdown is said as
  words; code blocks and tables are left to the panel.
- **A spoken turn tells the model it will be heard**: short spoken sentences, no Markdown, the
  language the user spoke, and that what it was given is a transcript that can be wrong. **It also
  does not think first** unless *Reasoning* is set to *On* outright: measured, gemma4:12b spent 21.6 s
  reasoning before a 58-character spoken reply, which in a conversation is 21.6 s of silence.
- **End of speech is a level detector** over 20 ms frames with a learned noise floor, running after
  the browser's own noise suppression and gain control (`src/lib/vad.ts`). While the assistant talks,
  a voice has to be 8 dB louder and 240 ms long to interrupt it. Echo cancellation removes what the
  page itself plays, which is what makes talking over it work at all; a Windows voice is played by
  the system rather than the page, so interrupting is off while one speaks.
- **The microphone prompt is WebView2's own.** Seelen's `wry` 0.55.1 answers only clipboard
  permission requests itself, so the first use asks. `npm run status` prints what the running webview
  reports - `microphone=prompt|granted|denied`, whether it has `AudioWorklet`, and the Windows voices
  it lists.
